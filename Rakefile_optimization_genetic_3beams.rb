...
def initialize_population_3beams (population, population_size)
...
def fitness_3beams (array_individual, array_A, array_B, array_C, nr_of_coat_bins)		# array_individual = [a, b]
	const_S1 = 		0.0
	const_S2 = 		0.0
	const_SDEV = 	0.0
	...
	(0..nr_of_coat_bins - 1).each do |i|										# iteration over the depth of the coating part of the target
		array_f[i] = 	a * array_A[i] + b * array_B[i]	+ c * array_C[i]						# f(z) is the overall distribution of defects made by a sum of the first distribution A(z) and the second distribution B(z)
		const_S1 += 	array_f[i]											
...
def optimization_plot_3beams(array_individual, file_DPA_1, file_DPA_2, file_DPA_3, folder_images, i_generation, coat_width, clad_width, nr_of_coat_bins, coating_type)
	...
	File.open("#{folder_images}/Generace_#{i_generation}.dat", "w") do |output|
		array_x_axis.each_with_index do |x_axis, i|
			...
			output.puts x_axis.to_s + "       " + (a * array_A[i] + b * array_B[i] + c * array_C[i]).to_s
		end
	end
...
desc "optimization based on genetic algorithms, for three ion beams"
task :optimization_genetic_3beams do
	...
	(0..nr_of_generations).each do |i_generation|										
		...
		(0..population_size - 1).each do |i|
			array_fitness[i] = fitness_3beams(population[i], array_A, array_B, array_C, nr_of_coat_bins)		# calculating fitness funcition for each individual in the population
		...
		# optimization_plot_3beams(population[best_fitness_index], file_DPA_1, file_DPA_2, file_DPA_3, folder_images, i_generation, coat_width, clad_width, nr_of_coat_bins, coating_type)
   end
end